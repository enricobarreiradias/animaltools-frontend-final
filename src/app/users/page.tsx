...
import { Security, Badge, Add, Edit, Delete, Block } from "@mui/icons-material";
...
export default function UsersPage() {
  ...
  const handleDelete = async (id: number, name: string) => {
    if (
      !confirm(
        `Tem certeza que deseja remover o usuário "${name}"? Essa ação não pode ser desfeita.`,
      )
    ) {
      return;
    }
              ...
              <FormControl fullWidth>
                ...
                {editingId === 1 && (
                  <Typography variant="caption" color="error">
                    O cargo do Admin Principal não pode ser alterado.
                  </Typography>
                )}
              </FormControl>
            )}
