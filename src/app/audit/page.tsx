...
export default function AuditPage() {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      if (usersList.length === 0 && uniqueUsers.size > 0) {
        setUsersList(
          Array.from(uniqueUsers, ([id, fullName]) => ({ id, fullName })),
        );
      }
    } catch (err) {
  ...
  const getIcon = (action: string, entity: string) => {
    if (entity === "ExternalApi" || action.includes("SYNC"))
      return <Sync color="info" fontSize="small" />;
    if (entity === "Evaluation")
      return <Assignment color="secondary" fontSize="small" />;
    if (action.includes("CREATE"))
      return <AddCircle color="success" fontSize="small" />;
    if (action.includes("DELETE") || action.includes("REMOVE"))
      return <Delete color="error" fontSize="small" />;
    if (action.includes("UPDATE"))
      return <Edit color="warning" fontSize="small" />;
                  ...
                  <TableRow key={log.id} hover>
                    ...
                    <TableCell>
                      <Box display="flex" alignItems="center" gap={1}>
                        <Person
                          fontSize="small"
                          color={log.user ? "primary" : "disabled"}
                        />
                        <Typography
                          variant="body2"
                          fontWeight={log.user ? "500" : "400"}
                        >
